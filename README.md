...
QC 담당으로 일하면서 재고를 확인할 일이 하루에 7~10건 정도 있었습니다. 그때마다
ERP에 로그인해 메뉴를 여러 단계 타고 들어가 품목을 검색해야 했고, 건당 1~2분이
걸렸습니다. 자주 보는 품목은 정해져 있는데 매번 같은 과정을 반복하는 것이 비효율적이라고
느꼈습니다.

또 BOM 구조, 부속품 단가, 협력업체 연락처, 입고 이력 같은 정보가 엑셀 파일과 메신저,
구두 전달로 흩어져 있어서 필요할 때 찾기 어려웠습니다.

그래서 ERP의 OpenAPI로 재고를 직접 가져와 자주 보는 품목을 한 화면에 띄우고,
흩어져 있던 정보를 한곳에 모으는 대시보드를 만들었습니다.
